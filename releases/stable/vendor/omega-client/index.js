...
async function main() {
  if (process.argv.includes("--check")) {
    ...
    const logger = new AgentLogger(config.logLevel);
    const errors = validateClientConfig(config);
    logger.info("RUNTIME", "Omega Client runtime check", redactedConfigSummary(config));
    if (errors.length > 0) {
      for (const error of errors)
    ...
    logger.info("RUNTIME", `Node ${process.version} ready`);
    logger.info("RUNTIME", "Filesystem and signal handlers available");
    return;
  }
  await runtime.start();
}
async function shutdown(signal) {
  if (!stopPromise) {
    const logger = new AgentLogger(loadClientConfig().logLevel);
    logger.info("AGENT", `${signal} received; shutting down safely`);
...
main().catch((error) => {
  const logger = new AgentLogger(loadClientConfig().logLevel);
  logger.error("AGENT", error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
